...
# Discription: A shell script application for automated machine learning analysis for MEG connectivity data, but with "cv only" method
		...
		case $opt in
			...
			i)
				...
				if [ ${MAT_FILENAME: -4} != ".mat" ]; then
					echo -e "${COLOUR_RED}\nERROR: -i the input file should be in .mat format.${NO_COLOUR}\n" >&2
					exit 1  # exit 1: terminating with error
				fi
...
if [ "$group_summary" == "none_existent" ]; then  # use "$group_summary" (quotations) to avid "too many arguments" error
	echo -e "${COLOUR_RED}\nERROR: -s or -y variables not found in the -a annotation file. Progream terminated.${NO_COLOUR}\n" >&2
	exit 1
else
