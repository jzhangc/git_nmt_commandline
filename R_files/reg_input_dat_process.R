...
if (!all(c(DAT_PROCESS_CONFIG$SAMPLEID_VAR, DAT_PROCESS_CONFIG$Y_VAR) %in% names(annot))) {
  cat("none_existent")
  quit()
}
...
if (any(is.na(unique(annot[, DAT_PROCESS_CONFIG$Y_VAR])))) {
  cat("na_values")
  quit()
}
if (length(unique(annot[, DAT_PROCESS_CONFIG$Y_VAR])) == 1) {
  cat("single_value")
  quit()
}

y <- annot[, DAT_PROCESS_CONFIG$Y_VAR]
...
save(DAT_PROCESS_CONFIG, file = paste0(DAT_PROCESS_CONFIG$MAT_FILE_NO_EXT, "_data_processing_config.RData"))
