...
# ------ config lists ------
FLAG_CONFIG <- list(DAT_FILE = args[6], 
                    MAT_FILE_NO_EXT = args[7],
                    RES_OUT_DIR = args[8],
                    CORE_OUT_OF_RANG = CORE_OUT_OF_RANGE,
                    PSETTING = PSETTING, 
                    CORES = CORES)
CFILE_CONFIG <- lsit(CPU_CLUSTER = args[11],
                     SVM_CV_CENTRE_SCALE = eval(parse(text = args[13])),
                     SVM_CV_KERNEL = args[14],
                     SVM_CV_CROSS_K = as.numeric(args[15]),
                     SVM_CV_TUNE_METHOD = args[16],
...
# }
if (CFILE_CONFIG$RANDOM_STATE) {
  set.seed(RANDOM_STATE)
}

# ------ set the output directory as the working directory ------
# setwd(RES_OUT_DIR) # the folder that all the results will be exports to
setwd(FLAG_CONFIG$RES_OUT_DIR) # the folder that all the results will be exports to

# ------ load and processed ML data files ------
ml_dfm <- read.csv(file = CFILE_CONFIG$DAT_FILE, stringsAsFactors = FALSE, check.names = FALSE)
...
error_flag <- NA
sink(file = paste0(CFILE_CONFIG$MAT_FILE_NO_EXT, "_svm_results.txt"), append = TRUE)
  ...
  tryCatch(
    {
      ...
      svm_nested_cv_fs <- rbioClass_svm_ncv_fs(
        x = nested_cv_x,
        y = nested_cv_y,
        univariate.fs = CFILE_CONFIG$CVUNI, uni.log2trans = CFILE_CONFIG$OG2_TRANS,
        uni.fdr = CFILE_CONFIG$UNI_FDR, uni.alpha = CFILE_CONFIG$UNI_ALPHA,
        uni.contrast = CFILE_CONFIG$CONTRAST,
        center.scale = CFILE_CONFIG$SVM_CV_CENTRE_SCALE,
        kernel = CFILE_CONFIG$SVM_CV_KERNEL,
        cross.k = CFILE_CONFIG$SVM_CV_CROSS_K,
        tune.method = CFILE_CONFIG$SVM_CV_TUNE_METHOD,
        tune.cross.k = CFILE_CONFIG$SVM_CV_TUNE_CROSS_K,
        tune.boot.n = CFILE_CONFIG$VM_CV_TUNE_BOOT_N,
        fs.method = "rf", cross.best.model.method = CFILE_CONFIG$SVM_CV_BEST_MODEL_METHOD,
        rf.ifs.ntree = CFILE_CONFIG$SVM_CV_FS_RF_IFS_NTREE, rf.sfs.ntree = CFILE_CONFIG$SVM_CV_FS_RF_SFS_NTREE,
        fs.count.cutoff = CFILE_CONFIG$SVM_CV_FS_COUNT_CUTOFF,
        parallelComputing = FLAG_CONFIG$PSETTING, n_cores = FLAG_CONFIG$CORES,
        clusterType = CFILE_CONFIG$CPU_CLUSTER,
        verbose = TRUE
      )
    },
    error = function(e) {
...
sink(file = paste0(FLAG_CONFIG$MAT_FILE_NO_EXT, "_svm_results.txt"), append = TRUE)
if (input_n_total_features > 1) {
  # plotting for rRF-FS
  cat("\n\n------ SFS plot error messages ------\n")
  for (i in 1:CFILE_CONFIG$SVM_CV_CROSS_K) { # plot SFS curve
    tryCatch(
      {
        rbioFS_rf_SFS_plot(
          ...
          plot.xLabelSize = CFILE_CONFIG$SVM_ROC_X_LABEL_SIZE,
          plot.xTickLblSize = CFILE_CONFIG$SVM_ROC_X_TICK_LABEL_SIZE,
          plot.xAngle = 0,
          plot.xhAlign = 0.5, plot.xvAlign = 0.5,
          plot.xTickItalic = FALSE, plot.xTickBold = FALSE,
          plot.yLabel = "OOB error rate",
          plot.yLabelSize = CFILE_CONFIG$SVM_ROC_Y_LABEL_SIZE, plot.yTickLblSize = CFILE_CONFIG$SVM_ROC_Y_TICK_LABEL_SIZE,
          plot.yTickItalic = FALSE, plot.yTickBold = FALSE,
          plot.rightsideY = TRUE,
          plot.Width = CFILE_CONFIG$SVM_ROC_WIDTH,
          plot.Height = CFILE_CONFIG$SVM_ROC_HEIGHT, verbose = FALSE
        )
...
svm_m <- rbioClass_svm(
  ...
  center.scale = CFILE_CONFIG$SVM_CV_CENTRE_SCALE, kernel = CFILE_CONFIG$SVM_CV_KERNEL,
  svm.cross.k = CFILE_CONFIG$SVM_CROSS_K,
  tune.method = CFILE_CONFIG$SVM_CV_TUNE_METHOD,
  tune.cross.k = CFILE_CONFIG$SVM_TUNE_CROSS_K, tune.boot.n = CFILE_CONFIG$SVM_TUNE_BOOT_N,
  verbose = FALSE
)
...
svm_m_cv <- rbioClass_svm_cv(
  ...
  center.scale = CFILE_CONFIG$SVM_CV_CENTRE_SCALE, kernel = CFILE_CONFIG$SVM_CV_KERNEL, cross.k = CFILE_CONFIG$SVM_CROSS_K, 
  cross.best.model.method = CFILE_CONFIG$SVM_CV_BEST_MODEL_METHOD,
  tune.method = CFILE_CONFIG$SVM_CV_TUNE_METHOD, tune.cross.k = CFILE_CONFIG$SVM_TUNE_CROSS_K, tune.boot.n = CFILE_CONFIG$SVM_TUNE_BOOT_N,
  parallelComputing = FLAG_CONFIG$PSETTING, n_cores = FLAG_CONFIG$CORES,
  clusterType = CFILE_CONFIG$CPU_CLUSTER,
  verbose = TRUE
)
...
if (input_n_total_features == 1 && CFILE_CONFIG$SVM_PERM_METHOD == "by_feature_per_y") {
  cat("WARNING: SVM_PERM_METHOD == 'by_feature_per_y' not valid with only one selected features. Set to 'by_y'.\n")
  CFILE_CONFIG$SVM_PERM_METHOD <- "by_y"
}

rbioClass_svm_perm(
  object = svm_m, perm.method = CFILE_CONFIG$SVM_PERM_METHOD, nperm = CFILE_CONFIG$SVM_PERM_N,
  parallelComputing = FLAG_CONFIG$PSETTING, clusterType = CFILE_CONFIG$CPU_CLUSTER, n_cores = FLAG_CONFIG$CORES,
  perm.plot = FALSE,
  verbose = FALSE
)
...
rbioUtil_perm_plot(
  perm_res = svm_m_perm,
  plot.SymbolSize = CFILE_CONFIG$SVM_PERM_PLOT_SYMBOL_SIZE,
  plot.legendSize = CFILE_CONFIG$SVM_PERM_PLOT_LEGEND_SIZE,
  plot.xLabelSize = CFILE_CONFIG$SVM_PERM_PLOT_X_LABEL_SIZE,
  plot.xTickLblSize = CFILE_CONFIG$SVM_PERM_PLOT_X_TICK_LABEL_SIZE,
  plot.yLabelSize = CFILE_CONFIG$SVM_PERM_PLOT_Y_LABEL_SIZE,
  plot.yTickLblSize = CFILE_CONFIG$SVM_PERM_PLOT_Y_TICK_LABEL_SIZE,
  plot.Width = CFILE_CONFIG$SVM_PERM_PLOT_WIDTH, plot.Height = CFILE_CONFIG$SVM_PERM_PLOT_HEIGHT,
  verbose = FALSE
)
